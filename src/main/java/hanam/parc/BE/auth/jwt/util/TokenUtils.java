...
public class TokenUtils {
     ...
     */
    public static String generateJwtToken(MemberResponseDto memberDto) {

        JwtBuilder builder = Jwts.builder()
                .setHeader(createHeader())                                  // Header 구성
                .setClaims(createClaims(memberDto))                           // Payload - Claims구성
                .setSubject(String.valueOf(memberDto.getId()))              // Payload - Subjects구성
     ...
     */
    private static Map<String, Object> createClaims(MemberResponseDto memberDto) {
        // 공개 클래임에 사용자의 이름과 이메일을 설정해서 정보를 조회할 수 있다.
        Map<String, Object> claims = new HashMap<>();

        log.info("loginId : " + memberDto.getId());
        log.info("username : " + memberDto.getName());

        claims.put(LOGIN_ID, memberDto.getId());
        claims.put(USERNAME, memberDto.getName());
        return claims;
    }
